...
\caption{Original collision meshes (a) provided by the robot manufacturer are often approximated by convex hulls, which are inaccurate while also more complex representations. \add{The mesh in b is an exact collision shape for the original gripper made of simple boxes primitives. We reduce the end effector complexity designing a simple single-finger gripper (c) made of only 2 boxes obtaining}\sout{In contrast, a simplified mesh (shown in (b) and (c)) can bring more accuracy as well as} a computational performance gain in terms of simulation rate.\label{fig:1}}
