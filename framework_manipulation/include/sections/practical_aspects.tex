...
\add{WE USE A DIFFERENT CONTROL LAW BLA BLA BLA}
\begin{equation} \label{eq:cmd_sim}
    \vect{\tau}_{cmd} = \coriolis \dconfigRobot + g(\configRobot) - \matr{K}_{D} \dconfigRobotError,
\end{equation}
with an appropriate choice of the positive-definite diagonal gain matrix $\matr{K}_{D} \in \nR{\robotDoF\times\robotDoF}$. 

\add{BETTER REPHRASE: WAS ADDED SOMEWHERE ELSE IN THE TEXT. ALSO ADD \cite{raisim_pd} WHICH IS REFERENCE TO RAISIM INTEGRATION SCHEME. Note that this control law differs from the one in \eqref{eq:cmd_sim}. 
...
\caption{Original collision meshes (a) provided by the robot manufacturer are often approximated by convex hulls, which are inaccurate while also more complex representations. \add{The mesh in (b) is an exact collision shape for the original gripper made of simple boxes primitives. We reduce the end effector complexity designing a simple single-finger gripper (c) made of only 2 boxes obtaining}\remove{In contrast, a simplified mesh (shown in (b) and (c)) can bring more accuracy as well as} a computational performance gain in terms of simulation rate.\label{fig:1}}
