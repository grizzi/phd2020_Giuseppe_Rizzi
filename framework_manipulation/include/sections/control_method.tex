...
\end{equation}
where $\epsilon$ is a positive minimum residual energy to avoid a singularity condition. We can discretize the previous equation to obtain a constraint which is affine in the input $\command$
\begin{equation*}
...
where we defined the residual energy $S(x_t(t))-\epsilon$ as $E_{res}$. The above inequality has the drawback that when the euler integration is performed with small time intervals, then a dangerously high power can be dissipated at any time, until very close to the lower energy limit. It is therefore better to use a value of $\alpha > 1/dt$. One can easily verify that defining a passivity ZBF as $h_{pass} = S(x_t) - \epsilon$ we obtain the same formulation. The ZBF constraint reads as: 
\begin{align}
    \dot{h}_{pass} &= \dot{S}(x_t) \\
     &\geq -\alpha h_{pass} \\
     &=\alpha (\epsilon - S(x_t)))
\end{align}
The passivity can therefore seen as a ZBF and consequently inherits its properties.
