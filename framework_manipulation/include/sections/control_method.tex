...
In this section we combine sampling, barrier functions and energy tank into a novel model-based control framework for robust and safe interaction control. We use the presented sampling method to generate joint velocity command sequences that optimize a task-dependent objective in a receding horizon fashion. We encode a set of tasks and safety related objectives as different cost terms. Nevertheless, sampling does not inherently provide guarantees and therefore we need a method to enhance safety. We address this problem using an optimization problem as a non-invasive way to change the command sequence when constraints are not fulfilled. In particular, we solve a \texit{sequential} optimization problem (QP), which takes as input a command sequence and starting state and returns a modified command-state sequence satisfying all constraints. Nevertheless, as the expensive rollout sampling procedure often limits the rate of the controller, a portion of this input sequence must be executed in \textit{open-loop} which can be unsafe in dynamic environments and for low sampling rates. On the other hand, the quadratic program can be efficiently solved at almost the same rate as the low-level controller. To this end, we propose a \emph{cascaded control} architecture composed of a low-rate policy update and a high-rate low-level control loop blocks. The latter solves the QP for the current state-input pair in order to find the best input according to the latest received measurements. In other words, the optimization problem is solved:
\begin{itemize}
    ...
    \item \textit{pointwise}, for each command executed by the low-level controller, in order to account for different control rates.
\end{itemize}
Sequential and pointwise optimizations are denoted as Sequential FILTER-QP and FILTER-QP respectively.
...
\begin{itemize}
    \item[$\Pi_{N}$:] relies exclusively on a stochastic controller to generate velocity commands, namely the \textit{Trajectory Filtering} block is completely missing and the sampled input trajectory is tracked as is, from the low-level velocity controller,
...
\centering
\hspace*{-1.5cm}
\includegraphics[width=1.3\columnwidth]{figures/schemes/receding_horizon_paper.pdf}
...
$n \gets 0$\;
\While{$n < T$}{
  $\bar{\command}_{t+n} \gets \text{FILTER-QP}(\vect{x}, \command, \boldsymbol{\tau}_{ext})$ \hfill (\eqn \ref{eq:cbf-qp}) \\
...
\end{equation}
with the auxiliary error variable $\configRobotError =  \configRobot - \configRobotDesired$. If we augment the the optimization problem in \eqref{eq:cbf-qp} with the energy constraints:
...
If the controlled system \eqref{eq:eom} satisfies the constraints \eqref{eq:pass_const} $\forall \ t$, then it is passive and therefore stable. 
\end{theorem}

\begin{corollary}
When the passivity constraint in \eqref{eq:pass_const} is included in the optimization problem of the FILTER-QP, then the controllers $\Pi_O$ and $\Pi_{IO}$ guarantee the stability of the system.
\end{corollary}


\begin{proof}
We start augmenting the system model with a virtual tank. We now need to define through which \emph{power ports} the tank exchange energy with the rest of the system. To this end, we perform a passivity analysis of the \emph{real} system. We first define the robot energy as, 
\begin{equation}
...
\end{equation}
Then, the energy evolution of the autonomous system, composed by robot, tank and environment is,
\begin{equation}
\begin{aligned}
...
\end{equation}
where $\epsilon$ is a positive minimum residual energy to avoid a singularity condition and complete tank depletion. This concludes the proof. 
\end{proof}
