...
\end{equation}
with the auxiliary error variable $\configRobotError =  \configRobot - \configRobotDesired$. \add{Note that this control law differs from the one in \eqref{eq:cmd_sim}. 
While the low level controller described in \eqref{eq:cmd_sim} is deployed in the simulated digital twin for generating and sampling interaction trajectories, law \eqref{eq:low-level-control} is deployed as a low-level controller on the real robot. There are some substantial differences between the simulated and real robot that require a modification of the low-level control laws:
\begin{itemize}
    \item In simulation gravity and coriolis compensation is perfect. 
    \item A full inverse dynamics controller relies on second order derivatives. As we use large simulation timesteps to improve sampling efficiency, these terms are very noisy and cause simulation instability.
\end{itemize}
 As a consequence in practice \eqref{eq:cmd_sim} suffices in the simulated physics for trajectory generation. 
On the real robot instead, gravity compensation errors are non negligible. Therefore the need for an additional integral term. Furthermore, dynamic compensation allows to guarantee the system passivity.  
These modifications generate a discrepancy between the real robot and simulated robot response. Note that these will never be the same, since a model is always a simplification of reality. In ultimate analysis we want the simulated model to be close to the real one, as long as it is still usable, meaning that we can efficiently generate rollout and that feasible solutions in simulation correspond to feasible solutions on the real robot.} 
...
Intuitively, the increase of the robot's energy is compensated with a reduction of the tank's energy. \remove{As the tank energy is limited and bounded from below, passivity is ensured if,}\add{Therefore we need to ensure that the tank is never completely depleted:}
\begin{equation}
