...
\commandTorque = \add{\massMatrix \dconfigRobotDesired} + \coriolis \dconfigRobotDesired + g(\configRobot) - \matr{K}_D \dconfigRobotError - \matr{K}_I \int_{0}^{\sigma} \dconfigRobotError\ dt,
\end{equation}
with the auxiliary error variable $\configRobotError =  \configRobot - \configRobotDesired$. \add{Note that this control law differs from the one in \eqref{eq:cmd_sim}. The additional dynamic compensation terms can be noisy and degrade the quality of the rollouts.} 
