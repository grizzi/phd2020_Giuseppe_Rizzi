...
\begin{equation} \label{eq:eom}
    ...
    \begin{bmatrix}
      \dconfigRobot \\
      \matr{M}_r^{-1}(\matr{J}_{r}^\transpose \vect{f}_{ext} - C(\configRobot, \dconfigRobot)\dconfigRobot - g(\configRobot) + \vect{\tau}_{cmd}(\command)) \\
      \dconfigObject \\
      \matr{M}_o^{-1}(-\matr{J}_{o}^\transpose \vect{f}_{ext} - C(\configObject, \dconfigObject)\dconfigObject - g(\configObject))
    \end{bmatrix}.
\end{equation}
